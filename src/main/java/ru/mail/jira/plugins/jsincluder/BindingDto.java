...
public class BindingDto {
  ...
  @XmlElement private boolean editContextEnabled;
  @XmlElement private boolean transitionContextEnabled;
  @XmlElement private String enabledContexts;
  ...
  private String buildEnabledContexts() {
    ...
    if (createContextEnabled) sb.append("Create");
    if (viewContextEnabled) sb.append(sb.length() > 0 ? ", " : "").append("View");
    if (editContextEnabled) sb.append(sb.length() > 0 ? ", " : "").append("Edit");
    if (transitionContextEnabled) sb.append(sb.length() > 0 ? ", " : "").append("Transition");
    return sb.toString();
  }
}
